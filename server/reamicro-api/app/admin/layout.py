...
def _admin_auth_shell(title: str, subtitle: str, body: str) -> str:
    ...
    return (
        ...
        f"<style>{ADMIN_AUTH_STYLE}</style></head><body>"
        f"<main class='panel auth-card'>"
...
def _admin_layout(
    ...
    version_line = f"<p class='muted api-tag'>API v{runtime.API_VERSION} · 时间均为北京时间</p>"
    return (
        f"<!doctype html><html lang='zh-CN'><head><meta charset='utf-8'>"
        f"<meta name='viewport' content='width=device-width,initial-scale=1'><title>{esc(title)}</title>"
...
def admin_error_page(status_code: int, message: str, request_id: str = "") -> str:
    ...
    body = (
        ...
        f"<p class='muted mb-lg'>{esc(hint)}</p>"
        f"<div class='inline'>{login_button}"
        f"<a class='button' href='/admin'>返回后台首页</a></div>"
        f"{request_note}"
    )
