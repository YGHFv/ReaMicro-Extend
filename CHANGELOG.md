...
新增 `ApiHighlightStylePayload.kt`，上传与安装共用同一份字段定义。九宫格图片路径**不上传**：
那是本机绝对路径，对别的设备毫无意义，带上去只会让对方拿到一个指向不存在文件的路径。
上传界面会提示有多少个样式引用了本机图片，说明对方只会得到颜色与 CSS。

### 顺带修掉的一个真 bug

安装高亮样式时**丢掉了全部深色主题字段**（`darkUsesLight`、`darkColor`、`darkFontFamily`、
`darkCss`、`darkNinePatchPath`、`darkNinePatchSlice`）。因为 `darkUsesLight` 的默认值是 true，
表现是深色模式下静默回落成浅色外观——不报错，但整套深色配置没了。现在浅色与深色字段
一起还原，并有往返测试锁住。

同时修了元数据推断：样式内容裹在 `{"schemaVersion":1,"style":{...}}` 里，此前没有下钻到
