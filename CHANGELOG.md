...
### 发现页：宫格「加载更多」闪退修复（2.3.6）

- **修复宫格模式连续两次「加载更多」闪退**（`ArrayIndexOutOfBoundsException: … length=-2`）。
  宿主 LazyList 的 item 按 key 复用，追加数据后原「末行」会原地从「2 本书 + 1 个占位块」
  变成「3 本书 + 0 个占位块」——孩子数量不变、但最后一格的节点**种类**从空格占位变成了
  书格。宿主这套 Compose gapbuffer 运行时在种类切换时会算错插入下标（按槽位下标而非
  真实孩子下标），数组拷贝长度变负直接崩。修复：每行**恒定产出 3 个同构书格**，
  缺书槽位用空白书格补齐，不再用 Spacer 占位。
- **宫格封面尺寸改用 `aspectRatio(3:4)`**（约束驱动），去掉按屏幕宽度反算的绝对高度，
  不再与宿主列表约束打架；封面宽度收起为格子的 `weight` 结果。
- 新增插入越界诊断探针（三层 hook `UiApplier.insertBottomUp` / `LayoutNode.insertAt` /
  `MutableVector.add`），越界时打印父容器与现有子节点清单，供后续同类问题定位。
...
- **单行省略改由 Compose 处理**：`renderDiscoverText` 单行组合改传 `TextOverflow.Ellipsis`
  （照抄在线补全搜索结果行已验证的实参：index 12 = overflow、`maxLines=1`、
  `changed=(0, 24960)`、`TEXT_SECONDARY_SINGLE_LINE_MASK`）。此前用 `fitText`（Paint 预量）
  做数据侧截断，测量字号/字体与真实渲染存在设备级偏差（声明 12sp 实渲约 10sp），
  截断点提前 20% 以上——同一书单在测试机正好顶到行末、在窄屏机上大片提前收尾。
- **修复纵向 spacer 失效**：`renderDiscoverSpacer` 挂的是 `width` 修饰符，放进 Column 后高度
  恒为 0，间距整个消失——网格封面与书名贴死（轻微重合）、列表四行挤成一片都是它。
  新增 `renderDiscoverVGap`（`height` 修饰符），列表/网格所有纵向间距改用它。
- 移除书单标题/作者/标签串的 Paint 预截断（保留源名一处：名字节点拿不到 `modifier`，
  无法用 weight 约束，长名会把布局按钮顶出屏幕）。

## 2026-09-22
## 发现页分页与行边距 - 2026-09-23

- **发现页支持翻页**：之前只请求分类地址的第 1 页（`{{page}}` 固定填 1），往下翻永远是这一页。现在 `DiscoverState` 记录每个分类已翻到的页码，书单末尾放「加载更多（已加载 N 本）」行，点击拉取下一页并按书 key 去重追加；翻到空页自动收起入口，失败可重试。滚动到底自动加载需要 LazyListState（反射拿不到），故用显式按钮。
- 列表行左右边距 16 → 8dp（叠加页面 16dp 后整体 24dp 对称），文字列可用宽度同步变宽。

## 发现页列表行重排 - 2026-09-23

- 列表详情改为四行：**书名 / 作者 · 最新：最新章节标题 / 状态 · 字数 · 平台 / 分类标签**，更新时间并入第四行语义（「最新章节」比日期更有用）。
- 修掉右侧边距过大：截断测量之前硬编码 12sp，而宿主 Typography 实际渲染的 bodySmall 更小，测量偏宽导致文字提前收尾。现在用 `TextStyle.getFontSize()` 读**实际字号**（解析 inline class `TextUnit` 的 `packedValue`：低 32 位 float bits、高 32 位类型，2=sp）再喂给 `fitText`，实测文字右端精确到达行右边界（1124px）。
- 列表行间隔收紧：行内垂直 padding 12 → 6dp，与 LazyColumn 的 16dp 项间距合计 28dp。

## 支持 Legado「JS 发现页」静态提取 - 2026-09-22

- 晚风里等聚合源的 `exploreUrl` 是一段 `@js:` 脚本（运行时生成「筛选/选择/快捷排序」分类），模块原本只认 JSON 数组与逐行两种写法，这类源会因解析不出分类而**从发现页直接消失**。
- 现在对这一形态做**静态提取**：`var groups=[...]` 是纯 JSON 数组，`search(params)` 的地址前缀与固定追加参数都是脚本字面量，分别用正则抓出来，再按 JSON 键序拼 `k=enc(v)`（空值跳过，与脚本语义一致），每个分组条目生成一个可点分类——晚风里解析出 60 个分类（排序 8 / 平台 23 / 分类 22 / 标签 7），接口实测可用。
- 参数顺序从**原始文本**按出现顺序提取：`org.json.JSONObject` 内部是 HashMap 键序会乱，而 JS `for...in` 跟的是插入序。
- 依赖 `source.getVariable()` 用户状态的选择行（`type:'select'`）与空地址标题行天然跳过。
- 分类名截断上限 12 → 20 字（「独阅读(cddaoyue)」这类平台名不再被截断）。

## 发现页第二轮修正：崩溃、标签与下载 - 2026-09-22

- **修掉发现页的偶发闪退**：列表模式曾经为了消除「每 6 本一条空白带」改成整张卡片一个 `LazyColumn` item，一次性插入几百个节点会触发 Compose 的 `UiApplier.insertBottomUp` → `MutableVector.add` 越界（`length=-3`）。改回「一本书一个 item」，节点数与封面并发都降下来，同时也没有了分组空白带。
- 书名/作者/标签串改为**按可用宽度自适应截断**：放得下的一律完整显示，只有真的超宽才截断加省略号（此前按固定字数截断，短书名也被砍尾巴）。网格与列表都适用。
- 列表标签串顺序对齐在线补全搜索结果：**状态 → 字数 → 章节数 → 平台 → 更新时间 → 其余**，分隔符也用 ` / `。
- **平台名走关联模块那张识别表**（`normalizedAssociationPlatformName`）：从分类词、详情/封面地址、书源名里识别出规范平台名（如 `qidian` → 「起点中文网」），识别不出就跳过这一段，不再直接把 `qidian 都市` 这类原始值当平台显示。
- 小标签与未选中的分类胶囊**去掉底色**（宿主主题下 `surfaceVariant` 是淡黄色），选中的胶囊改为把圆角 `shape` 显式传给 `background`——此前靠 `clip` 裁剪，底色实际是直角块。
- 点击图书条目**接入在线下载流程**：弹出确认弹窗（整本下载 / 逐章加载 / 取消），再交给宿主那条链路（限流、通知、任务记录都在那边）。此前是静默开始，加上部分 ROM 会吞掉 Toast，体感就是「点了没反应」。
- 注入页导航改用**最新的 NavGraphScope**：Activity 被系统回收后重建时，缓存里可能还留着已销毁组合的旧实例，用它 `navigate` 会静默失败（现象：从多任务切回来后再点「发现」点不动）。现在借导航图谱重组时的 `composable(...)` 调用刷新缓存，并在多个候选 scope 间依次重试。

## 发现页改版：书源弹窗、标签折叠与网格布局 - 2026-09-22

- 「发现」页重排版式：标题栏下新增「书源名 ▾」行，点名字开书源弹窗（带筛选框与勾选标记）；右上角新增布局按钮，在列表与网格之间切换，选择结果持久化到偏好文件。
- 分类标签改为**永远一行**：按标签实际字号预量宽度，放得下的才渲染，放不下的折进末尾的「▾」展开按钮，点开是三列分类选择弹窗。
- 书单支持两种排布——列表版（封面 + 书名 + 作者·日期 + 字数/分类/状态小标签）与三列网格版；封面复用在线补全的封面加载器，带书源 header 与登录态。
- 状态标签改用在线补全那套综合解析（中文/数字/布尔、`creation_status`、`is_finish`，以及从「最新章节」标题推断），不再把源里的数字状态码原样显示成一个孤零零的「1」。
- 书源弹窗的源标识剥掉内部的 `online_` 前缀与超长哈希别名。
- 新增自绘界面图标（下拉三角 / 列表 / 网格），并把 `ImageVector` 的反射构建流程抽成可复用的 `DiscoverVectorBuilder`。
...
- 在线源搜索结果行的城市/作者/元信息改用全局字体：这些行是原生 `TextView`，原本不在 Compose 与 Dialog 的覆盖范围内。
- 每日轶闻把奖励明细（阅历/彩筹/期物）写进通知正文与任务记录，通知就地给物品名上品质色，不点开也能看到领到什么；自动祈愿同样带结构化物品。
- 自动行商的新行商消息带结束时间和新运签效果，并在记录详情中单独列出。
- 期物典当的「禁当期物」可按任务配置（每行一个 propId）；旧配置沿用默认清单，显式清空表示不禁止任何期物。
...
## KSU 下载入口与 CI 打包 - 2026-09-15

- CI 跟随 APK、KSU 脚本和打包器变更生成配套模块 ZIP，上传独立 Actions artifact，并与 APK 附到同一条 CI 预发布。
- 模块 ZIP 文件名读取 `module.prop` 版本，只打包六个必需文件；增加编码、权限、版本、文件白名单和可复现性测试。
- 模式说明增加 GitHub Releases 下载入口，明确两种模式无需重新配置；切换仅等待本机执行落盘，不等待行商旅程或轶闻解锁。保留原有防双跑交接机制。

## 自动任务审查、隐藏后台卡片与 KSU 实验模式 - 2026-09-15

- 本地轶闻领奖接入祈愿事件，失败保留待执行标记；云端联动祈愿走正常任务锁、退避和通知链路。
- 修正多本阅读的累计和每日上限，保留部分成功进度；典当 API 接纳 `pawn`，修正失败计数和 `maxRetries=0`。
- 本地网络执行移入 `JobService`，补充关键进度检查点、中断后停止新请求，以及云端网络返回覆盖最新配置/暂停/删除的竞态保护。
- Android/KSU 共用本地引擎；新增 root 私有状态、保守执行权交接、排队请求、记录同步、重排和通知回执。KSU 模式为实验性，不默认接管。
- 修复 Android 不支持 `Files.getFileStore()` 导致的独立入口崩溃，普通 shell 自检验证加载、持久化、脱敏和文件权限。
- 模块配置「权限与后台」新增隐藏后台卡片开关，离开界面后隐藏最近任务卡片，不强停应用或切换任务执行模式。
- 增加 [完整逻辑审查](docs/automation-audit.md) 与 [KSU 安装/回退说明](ksu/README.md)；root 接管与长时间熄屏执行仍待实机试用验证。

## 本地与云端自动任务：修正求签、领奖、续开与唤醒同步 - 2026-09-15

继续核对已有修复，并通过设备任务记录、系统闹钟状态及 MT MCP 的阅微 2.3.2 beta 宿主实现确认接口契约。

### 求签遵循配置与实际操作阶段

- 每日轶闻不再强制求运；「不祈禳」明确传空签种，不发祈禳请求。
- 模块、本地与云端编辑器共用签种解析，重新打开或保存其它配置时不再把「不祈禳」重置成默认签种。
- 配置的签种与账户现有签不同才替换，同种沿用；校验祈禳返回的实际签种与 `success/message`，不再虚报成功。
- 求运发生在当天首次生成轶闻之前；行商只在准备开启新行商时祈禳，在途查询和领取奖励不重复消耗道具。
- 配置祈禳失败时保留任务并重试，不继续执行需要该运签的操作。

### 轶闻按真实结束时间领取

- `get-daily-lore` 获取轶闻及 `endTime`，`isFinish` 表示能否领奖，不是「是否已经签到」。
- 未结束只保存轶闻 ID 与结束时间；可领取后才调用 `complete-daily-lore`，同一轮不重复调用。
- 服务端时间优先于旧缓存；可领取状态不会被历史错误的未来时间挡住。
- 没有结束时间时每小时继续查询；未解锁或暂时失败保留领取进度并安排重试，不直接跳到次日。

### 行商先领取再续开

- 宿主的 `SETTLED` 表示结果已生成、奖励待领取，仍需调用 `settle-traveling-merchant`。
- 通知去重、领取完成、续开成功分别记账；已经通知的行商仍可自动领取，续开失败可重试且不重复领取。
- 校验领取和开启响应里的 `success`，失败原因如实显示；解析响应的 `data.trip`，不再误判成功或丢失结束时间。
- 新行商按真实 `endTime + 60 秒` 检查；预计时间已到但服务端尚未结算时短时复查。
- 允许宿主合法的 `transportId=0`（无车马），沿用当前行商参数时不会把零号车马换回旧车马。

### 轶闻详情完整展示

- 执行记录保存完整轶闻正文、出处、品质、等级、阅历、彩筹、期物以及开始、结束、领取时间；暂未解锁或领取失败也保留已获取的详情。
- 详情正文可滚动、选择，轶闻标题按 `quality` 着色，期物按独立的 `propQuality` 着色；长正文不截断。
- 旧记录没有保存的正文不做猜测，并提示重新查询今日轶闻；记录桥接同步保留完整详情。

### 配置、执行状态与唤醒保持一致

- 镜像合并兼容缺失或显式为空的可选字段，修复 `LocalTaskMirrorReceiver` 接收配置时的空指针崩溃。
- 停用云端或清空服务器地址后清除旧的云任务唤醒缓存，本地任务仍正常排程。
- 本地执行和闹钟均由模块进程负责，并取消宿主遗留的重复闹钟；本地执行入口防并发重入。
- 到期任务安排短时唤醒，不能因时刻已经过去而被忽略到次日零点。
- 云任务时刻持久化；本地重排或网络失败不清空已知云任务时间，服务器明确无任务时才清空。
- 两处设置按配置版本合并，旧宿主镜像不能覆盖模块里刚保存的「不祈禳」或停用选择。
- 镜像完成后，通过已有调用方校验的 Provider 回读模块执行状态，使阅微设置页与模块页面显示同一份进度；返回数据不包含凭据。
- 更新后对旧轶闻、行商任务做一次状态复查，清除不可信的旧成功标记；不重启停用或暂停的云任务。
- 新增本地 HTTP 流程测试、配置合并与唤醒回归测试，并同步覆盖云端相同的接口与状态问题。

模块 versionCode 为 63，versionName 维持 2.3.2；云端修复需部署更新服务器代码后生效。

## 主界面改三页签 + 任务记录详情 + 修正「下次执行时间」 - 2026-09-14（同日第四批）

### 主界面重构为底栏三个页签

- **记录**（首页）：只列任务记录，按系统通知的样式呈现（任务名 + 时间 / 正文），整块可点；
  点进去是详情页——时间、任务、账号、结果，以及这次执行的细节。
- **配置**：任务概览（账号数/已启用数/下次唤醒/下次任务时刻）、每个本地任务一张卡片
  （启用停用、改配置）、权限与后台（通知/精确闹钟/电池优化/厂商自启动/重排闹钟）、通知记录、模块日志。
- **关于**：版本、Root 增强开关与状态、模块进程 PID、通知权限、下次唤醒。

任务配置在模块侧也能改了（执行时间、阅读时长与图书、祈愿上限、行商城池/本金/车马、运签）。
**注意**：这里改的是模块进程执行用的那份本地任务配置；阅微设置页里的改动会在下次下发时覆盖它，
界面卡片上已写明这一点。

### 任务记录带详情

`LocalTaskRecord` 增加 `detail`（`{标签: 值}` 紧凑 JSON），执行器为每个任务填上用户真正关心的东西：

- 每日轶闻：奖励领取状态 + 运签（签种/签文名）与**运签效果原文**（例如「下一次每日轶闻：绿色及以上
  概率提升 2 个百分点」）；
- 自动行商：事件、行程、城池、本金、结算、状态与收益/亏损；
- 自动祈愿：本次祈愿次数与获得物品；自动阅读：图书与时长；期物典当：期物、数量、铜钱、今日次数。

### 修正「下次执行时间」与实际不符

原来显示的是 `earliestNextRunAt()`——任务自己排的 nextRunAt 的最小值。它有两个问题：可能**已经过期**
（实际会在下次唤醒时立刻执行），而且和真正触发执行的**系统闹钟时刻**根本不是一回事。现在分开显示：
**下次唤醒**取模块写下的闹钟触发时刻（`NextWakeHint`，即真正会触发执行的时刻），
**下次任务时刻**只取未来的 nextRunAt；过期的值不再被当成「下次执行」。

模块 versionCode 更新为 62（versionName 维持 2.3.2）。

## 自动任务：道观祈禳、期物典当、自动行商修复、任务改名 - 2026-09-14（同日第三批）

用户提的一批自动任务改动，本地（模块）与云端（server）两路都做。

### 道观运签（任务前检查，没有才祈禳）

- 新增 `ensureTaoistBlessing`：跑任务前先 `get-taoist-blessing` 看有没有签，**没有才**
  `pray-taoist-blessing` 祈禳一支——已有签不替换，替换会白白消耗祈禳道具。签种 wire 值取自宿主
  `ui/shrine/components/TempleWay`：`LUCK`（求运）/ `SAFETY`（求安）/ `WEALTH`（求财），
  自造枚举会被服务端判为非法。
- 覆盖范围按用户指定只两个任务：**每日轶闻固定求运**；**自动行商可在求安/求财之间选**（任务弹窗里切换）。
- 任务消息会带上「已祈禳求运签」或祈禳失败原因，方便回查这次任务为什么没有签加成。

### 新增任务「期物典当」

按参考脚本实现：`get-pawn-count` 取当日可典当期物与剩余次数 → 命中禁当清单（propId 11-18，都是
祈禳/传承/夺宝要用的消耗品）就跳过 → `get-user-materials` 找到该期物的 `userPropId` → 循环 `pawn`
直到次数或持有量用尽。典当次数用尽、期物不可当、背包没有，都按「跳过」处理（不当作故障、不打扰用户）。

### 修复自动行商

两个报障其实是**同一个根因**：游戏在 `endTime` 到达时就把行商状态置成 `SETTLED`，而原代码在
SETTLED 分支只说一句「行商已结算」就按 4 小时重排——所以 ① 通知里既没有事件也没有收益/亏损，
② 检查节奏退化成固定 4 小时轮询（而不是按行商时间）。

- SETTLED 分支现在按趟去重后**如实播报**「事件 · 收益/亏损」，并据此排程；
- **自动开始行商不工作**：原来只在「抵达且自动结算成功」这一条路径上开新行商，链路一旦在中途断掉
  （比如用户手动结算过一次）就再也开不起来。现在没有在途行商且开启自动完成时会补开一趟；
  结算后开新行商也支持「城池/本金/车马留空沿用上次行商配置」——服务器侧此前完全没有这个沿用逻辑
  （模块侧有），所以云端任务只要没手填参数就永远开不了新行商；
- 服务端同样补上「上次行商配置」的落盘与沿用（`merchantLastCityCode` 等）。

### 任务改名

野社零点签到 → **每日轶闻**；野社自动抽卡 → **自动祈愿**；行商通知 → **自动行商**；
云端自动阅读 → **自动阅读**。模块设置页（云端页/本地页共用一份任务规格）、通知标题、模块主界面、
服务器任务标签与后台页面全部同步。

单测：模块 530 项、服务器 437 项全绿（新增 4 + 14 项覆盖祈禳、典当与行商结算播报）。

模块 versionCode 更新为 61（versionName 维持 2.3.2）。
...
- 本地与云端同一任务的启用状态互斥：启用某任务的本地版本会自动关闭该任务的云端版本，反之亦然。
- 新增「行商通知」任务（本地与云端都支持）：每 4 小时检查一次行商状态，检查到行商在途时暂停轮询到完成时刻，完成后通知事件标题与收益/亏损；默认不自动完成，可开启「自动完成行商」并配置城池/本金/车马以自动开启新行商。
- 服务器端 `executors`/`scheduler`/`tasks` 增加 `traveling_merchant` 任务类型与设备执行支持；设备完成回报新增 `notify` 标志，行商在途等中间态只更新状态不打扰用户。
- 模块版本更新为 2.3.2。

## 修复野社自动祈愿接口 - 2026-08-29

- 按阅微 2.3.1 当前实现将旧的 `rest/lottery/lottery-v2` 替换为 `rest/community/wish`，避免业务码 13“活动已结束”。
- 按本次剩余次数组合九连抽和单抽；配置为 0 时先读取真实彩筹余额，再用相同规则清空余量。
- 回执改为解析新接口的 `data.props`，逐项显示本次祈愿得到的名称和品质。
- 热修复版本更新为 2.0.2。

## 野社抽卡上限与具体结果回执 - 2026-08-29

- 野社自动抽卡支持填写每日上限，默认 3 次；配置为 0 时会先读取阅微返回的真实彩筹余额，再在签到奖励领取后全部抽完。
- 有限上限会一次补足当天剩余抽卡次数，每次成功后立即记录计数，避免中途失败造成重复抽取。
- 抽卡通知逐次显示奖品名称、品质和数量，彩筹不足、业务错误和部分成功也会返回明确结果。
- 模块版本更新为 2.0.1。

## 云端任务独立设置页与阅读书目回执 - 2026-08-29

- 云端任务改为独立列表页，分别展示野社签到、自动抽卡和云端自动阅读；首次启用先填写当前任务配置，已有配置时开关直接启停。
- 每项任务可单独编辑执行时间，自动阅读可继续配置阅读分钟数、最近阅读或多本指定图书，不再由一个总弹窗同时覆盖三项任务。
- 野社自动抽卡改为签到奖励领取完成后的事件联动任务，不再保存或展示每日执行时间，也不会在零点独立运行。
- 任务列表安全回填执行参数，只公开阅读时长与书目，不下发阅微密钥或原始请求。
- 云端自动阅读通知会显示本次实际上报的具体图书，例如“云端自动阅读完成 1 本：测试图书；累计 30 分钟”。

## 修复野社签到奖励领取 - 2026-08-29

- 按阅微 2.3.1 实际实现修正领取链路：签到和领取奖励都调用
  `rest/community/complete-daily-lore`，并传同一个 `userLoreId`；不再误查通用任务中心列表。
- 使用每日轶闻响应中的真实 `endTime` 安排领取；接口暂未解锁时 5 分钟后继续检查，不会等到次日。
- 领取成功或检测到此前已领取时，通知会附带具体奖励，例如“阅历 5 点、彩筹 3 枚、端砚”。

## 修复云端自动阅读读取最近图书 - 2026-08-29

- 按阅微 2.3.1 的实际接口修正最近阅读和时长上报路径。
- 云端自动阅读只上报阅读时长，不调用阅读进度接口，不改变书籍 CFI、章节或百分比进度。
- 检查阅微响应内的业务码，HTTP 200 的接口错误不再误报为“没有找到可阅读的图书”。

## 在线章节标识只存 ID，不再重复完整 URL - 2026-08-28

`reamicro-online-chapters.json` 里每一章的 `sourceChapterId` 存的都是完整 URL：

    "sourceChapterId": "https://book.whispertale.fun/reader-api/books/1039831421/chapters/789864061"

域名和接口路径对识别章节没有贡献，却在每一章里重复一遍。现在只存能唯一定位章节的那一段：

| URL 形态 | 标识 |
|---|---|
| `.../books/1039831421/chapters/789864061` | `789864061` |
| `read.php?bookid=1&chapterid=9527` | `chapterid:9527` |
| `.../read/1001/5` | `1001/5` |
| `.../book/12345678.html` | `12345678` |

抓正文用的完整地址仍然存在 `url` 字段——那个是功能需要，不能省。

几处刻意的处理：

- **查询参数优先于路径**：查询参数是书源显式声明的章节 ID，比从路径推断可靠。
- **分页式路径带上前一段**：`.../read/1001/5` 取成 `1001/5` 而不是 `5`，否则不同书的
  同号章节会撞成同一个标识。
- **认不出 ID 时退回完整 URL**：路径末段是 `read` 这类词而非 ID 时，宁可长一点也不能让
  多个章节共用一个标识。
- 不透明段（哈希、UUID）要求至少 8 位才当作 ID，避免把 `txt`、`html` 之类当成标识。

### 向后兼容

`sourceChapterId` 是**更新时对齐已存章节与远端章节的匹配键**。已下载图书存的是旧格式完整
URL，新算出来是短 ID，直接比对会全部对不上，更新时每一章都被判成新章节重新下载。

所以新增 `sourceChapterMatchKey()`：比对前两边都归一，完整 URL 压成同样的短 ID，短 ID 原样
返回。已下载的图书不需要重新下载，也不会因为这次改动触发全量重下。


## 数据保留策略 - 2026-08-28

三处数据此前**无限增长**，长期运行会把数据卷占满。原先只有服务器快照（默认 30 份）和任务
消息（每用户 200 条）有上限：

| 数据 | 原先 | 现在 |
|---|---|---|
| 审计日志 `events.jsonl` | 只追加，永不轮转 | 超过 32 MB 切历史文件，最多留 3 份 |
| 任务日志 `{task_id}.log` | 只追加 | 单个超过 512 KB 裁掉前半段 |
| 内容包 `history/` | 每次编辑归档一份完整副本 | 每个内容包最多留 10 份 |

新增 `app/retention.py`。几处刻意的取舍：

- **审计切历史文件而不是截断**。审计记录是安全事件，直接丢掉最早部分不合适；`events.jsonl.1`
  是最近切下来的，编号越大越旧。只有把保留份数明确配成 0 才直接删。
- **任务日志直接裁前半段**，不留历史文件——每个任务都留会产生大量小文件。裁剪后首行插一句
  说明，否则看起来像日志丢了。裁剪按字节回读，会丢弃可能被截断的半行。
- **归档时立刻裁剪**，不等定时清理：编辑频繁的源会在两次清理之间堆出很多副本。上传与后台
  编辑是两条独立的归档路径，两处都加了。
- 清理搭在定时快照的同一个低频循环里，不额外起后台任务；放在快照之后，先留档再清理。

后台设置页新增四个阈值输入框，页面下方列出各类数据的**当前占用**——光有阈值看不出离上限
还有多远。可手动触发一次清理，不必等低频循环。

### 顺带修掉一个 bug

`delete_task` 在移除任务**之后**还调 `task_log(task_id, "任务已删除")`，这会给已经不存在的
任务**新建**一个日志文件——正是孤儿日志的来源。现在改为删除该任务的日志文件。


## 书源规则级检测 - 2026-08-28

地址检测只验证域名可达，但站点最常见的失效方式是**改版**：域名还在、首页 200，搜索接口的
字段结构变了，`ruleSearch` 取不到任何东西。这类源在列表里显示"可用"，实际已经搜不出书。

现在支持规则级检测：按书源自己的 `searchUrl` 与 `ruleSearch` 真发一次搜索，看还能不能取出
书名。判定分四类：

| 判定 | 含义 |
|---|---|
| 规则可用 | 规则命中，取到了书名 |
| 规则失效 | 请求成功但规则取不到条目，站点大概率改版了 |
| 无法检测 | 规则依赖 JS 脚本，服务端没有 JS 引擎，**不算失效** |
| 不可达 / 地址被拒 | 沿用地址检测的判定 |

新增 `app/jsonpath.py`：模块端 `OnlineJsonPathCompat.kt` 的 Python 移植。**语义必须与模块
一致**——服务端按这套语义判定"规则还能不能取到数据"，两边解析结果不同，检测结论就没有意义。
覆盖字段路径、数组下标与通配、`*` / `*[*]` 子节点、`$..` 递归下降（且下降后继续执行剩余路径，
不把 `$..name[*]` 截断成 `$..name`）、`||` 取首个非空、`&&` 合并全部。刻意不支持过滤表达式与
脚本：书源里用得少，且服务端没有 JS 引擎，遇到时按"无法检测"处理而不假装成功。
展开量有上限，避免畸形规则在大响应上炸开。

新增 `app/rule_check.py`：解析 `searchUrl`（含 legado 风格的 `,{"method":"POST","body":...}`
选项块与 `{{key}}` 模板）、发请求、按规则解析响应。JSON 走移植的 JSONPath，非 JSON 退化为
HTML 锚点扫描。**SSRF 防护**沿用地址检测那一套：目标 URL 来自上传的书源内容，等于让服务器
访问任意地址，所以只允许 http/https、拒绝内网与回环、不跟随跨主机跳转、限响应体与超时。
选项块 JSON 写坏时按纯 GET 处理，不因为附加参数有问题就判整个源失效。

后台内容列表新增"规则可用性"列（与"地址可用性"并列，都读清单缓存不触发网络请求），
书源行提供"检测规则"按钮，分区提供"检测全部规则"并可自定义试搜关键词（默认「剑」）。
批量检测串行执行，避免同时向多个站点发搜索请求。


## 上传配额执行与内容包批量操作 - 2026-08-28

### 上传配额从"能配但不生效"变成真正生效

用户档案里的 `uploadQuota` 字段此前只能存能读，**没有任何强制逻辑**——管理员填了数字也不会
拦住任何上传。现在补上执行：

- 新增 `users.check_upload_quota()`：按 `uploadOwner` 统计该用户已拥有的内容包数量，
  折叠历史归属写法（`host-public:3`、`key:xxx:host:3` 都算同一个用户），达到上限返回 409
  `UPLOAD_QUOTA_EXCEEDED` 并附用量明细。
- **只拦新建**。命中已有内容包走关联路径，不占额度——否则用户想更新自己的源会被自己的配额
  挡住。后台上传的内容包没有归属，不计入任何用户配额。删除内容后额度自动释放。
- `/v1/packages/upload/policy` 一并返回 `quota` / `used` / `remaining`，模块在上传前就能显示
  还能传几个，不用先撞墙再看报错。
- 后台用户页新增"上传上限"输入框，使用情况列显示 `已用/上限`（不限时显示"不限"）。

### 内容包批量操作

上百个内容包此前只能逐个点下架、删除、检测。现在内容列表每行带勾选框，顶部批量操作条支持
**批量上架、下架、删除、检测地址**。

几处刻意保守的地方：批量删除**不放宽**单个删除的规则，已发布内容仍必须先下架；上架仍校验
依赖是否满足；检测串行执行，避免同时打爆目标站点；批量操作只在具体类型分区提供，不做跨类型
批量以免误操作。单个条目失败或被跳过不影响其余条目，结果按"成功／跳过／失败"分类汇总回报。
...
九宫格图片会以内嵌文件随样式上传：载荷记录文件名、MIME、SHA-256 与 Base64 内容，不携带
本机绝对路径。其他设备安装时会校验 PNG 签名与哈希，按图片哈希恢复到模块私有目录并写回
`ninePatchPath`；相同图片重复安装会复用已有文件。旧版不含图片的载荷仍可正常安装。
